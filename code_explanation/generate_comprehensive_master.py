...
# Comprehensive Python script to generate complete line-by-line breakdowns
# and beginner concepts for EVERY file from 1 to 11.

def generate_full_dataset():
    modules = []

    # -------------------------------------------------------------
    # 1. INGESTION LOADER (src/ingestion/loader.py)
    # -------------------------------------------------------------
    modules.append({
        "id": "ingestion-loader",
...
from langchain_core.documents import Document""",
    ...
    # -------------------------------------------------------------
    # 2. INGESTION CHUNKER (src/ingestion/chunker.py)
    # -------------------------------------------------------------
    modules.append({
...
class FinancialVectorStore:
    ...
    # -------------------------------------------------------------
    # 4. BM25 RETRIEVER (src/rag/bm25_retriever.py)
    # -------------------------------------------------------------
    modules.append({
...
class FinancialBM25Retriever:
    ...
    # -------------------------------------------------------------
    # 5. HYBRID RETRIEVER (src/rag/hybrid_retriever.py)
    # -------------------------------------------------------------
    modules.append({
...
class FinancialHybridRetriever:
    ...
    # -------------------------------------------------------------
    # 6. DETERMINISTIC CALCULATOR (src/tools/calculator.py)
    # -------------------------------------------------------------
    modules.append({
        "id": "tools-calculator",
...
def calculate_efficiency_ratio_tool(non_interest_expenses: float, total_net_revenue: float) -> str:
    ...
    # -------------------------------------------------------------
    # 7. MARKET DATA TOOL (src/tools/market_data.py)
    # -------------------------------------------------------------
    modules.append({
...
def get_market_data_tool(ticker: str) -> str:
    ...
    # -------------------------------------------------------------
    # 8. SCHEMAS & STATE (src/schemas/financial_state.py)
    # -------------------------------------------------------------
    modules.append({
...
class FinancialMetricItem(BaseModel):
    ...
    # -------------------------------------------------------------
    # 9. GUARDRAILS (input_guardrails.py & output_guardrails.py)
    # -------------------------------------------------------------
    modules.append({
        "id": "guardrails-input",
...
class FinancialDossierResponse(BaseModel):
    ...
    # -------------------------------------------------------------
    # 10. MULTI-AGENT GRAPH & NODES (src/agents/graph.py & nodes.py)
    # -------------------------------------------------------------
    modules.append({
        "id": "agents-graph",
...
def run_financial_analysis(
    ...
    # -------------------------------------------------------------
    # 11. EVALUATION (src/evaluation/benchmark.py)
    # -------------------------------------------------------------
    modules.append({
...
class RAGEvaluationBenchmark:
    ...
    # -------------------------------------------------------------
    # 12. API & UI (src/api/main.py & src/ui/app.py)
    # -------------------------------------------------------------
    modules.append({
        "id": "api-main",
...
def test_division_by_zero():
    ...
    # -------------------------------------------------------------
    # 14. FACTUAL GROUNDING (Category 10)
    # -------------------------------------------------------------
    modules.append({
...
python-dotenv>=1.0.1""",
    ...
    # -------------------------------------------------------------
    # 16. END-TO-END SYSTEM STORY & PIPELINE ARCHITECTURE (Category 12)
    # -------------------------------------------------------------
    modules.append({
...
all_mods = generate_full_dataset()
js_code = f"// MASTER FINAGENT CODE EXPLANATION DATASET\nexport const PROJECT_MODULES = {json.dumps(all_mods, indent=2)};\n"

# 1. Update in code_explanation/src/data/projectData.js
with open("/Users/narendramishra/GEN AI /code_explanation/src/data/projectData.js", "w", encoding="utf-8") as f:
    f.write(js_code)

# 2. Update in FinAgent/code_explanation/src/data/projectData.js
with open("/Users/narendramishra/GEN AI /FinAgent/code_explanation/src/data/projectData.js", "w", encoding="utf-8") as f:
    f.write(js_code)

print(f"Successfully generated comprehensive dataset with {len(all_mods)} fully annotated modules in both locations!")
